...
export function ballHitTopOrBot(ball, arenaHitbox) {
  if (
    ball.hitbox.max.z >= arenaHitbox.max.z ||
    ball.hitbox.min.z <= arenaHitbox.min.z
  )
    ball.direction.z *= -1;
}
...
function isBallBetwinArenaPlayerTop(ball, player, arenaHitbox) {
  if (
    ball.hitbox.max.z >= arenaHitbox.max.z &&
    ball.hitbox.min.z <= player.hitbox.max.z
  )
    return true;
  return false;
}
...
function isbBallBetwinArenaPlayerBot(ball, player, arenaHitbox) {
  if (
    ball.hitbox.min.z <= arenaHitbox.min.z &&
    ball.hitbox.max.z >= player.hitbox.min.z
  )
    return true;
  return false;
}
...
function isBallBehindPlayer(ball, player) {
  if (player.type == "left") {
    if (
      (ball.hitbox.min.x >= player.hitbox.min.x &&
        ball.hitbox.min.x <= player.hitbox.max.x) ||
      (ball.cube.position.x >= player.hitbox.min.x &&
        ball.cube.position.x <= player.hitbox.max.x)
    )
      return true;
  } else if (player.type == "right") {
    if (
      (ball.hitbox.max.x >= player.hitbox.min.x &&
        ball.hitbox.max.x <= player.hitbox.max.x) ||
      (ball.cube.position.x >= player.hitbox.min.x &&
        ball.cube.position.x <= player.hitbox.max.x)
    )
      return true;
...
export function ballPinch(ball, player, arenaHitbox) {
  if (
    isBallBetwinArenaPlayerTop(ball, player, arenaHitbox) &&
    ...
    isbBallBetwinArenaPlayerBot(ball, player, arenaHitbox) &&
    isBallBehindPlayer(ball, player)
  ) {
    let newDirection = new THREE.Vector3(1, 0, 0.5);
    if (player.type == "right") newDirection.x *= -1;
...
export function ballAntiBlockSystem(ball, player) {
  if (
    ball.cube.position.x == player.cube.position.x &&
